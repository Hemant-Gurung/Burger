...
void StartScreen::Initialize()
{
	...
	//RIGHT
	dae::Input::GetInstance().MapEvent(std::make_pair(dae::XBOX360Controller::ButtonState::down, dae::XBOX360Controller::ControllerButton::ButtonA), InputAction(std::make_unique<Button>(gameObjectStart), VK_SPACE, GamePadIndex::playerOne));
	//LEFT
...
void StartScreen::Update(float dt)
{
	...
	if (dae::InputManager::GetInstance().IsDown(static_cast<unsigned int>(dae::XBOX360Controller::ControllerButton::ButtonA), indx))
	{
		ClearScene();
	}
